...
export default function CartViewScreen() {
  const { setCurrentScreen, cartItems, updateCartItemQuantity, removeCartItem, setCartItems } = useApp();
  ...
  // Background colors for new items
  const bgColors = ['#FEF2F2', '#EFF6FF', '#FEFCE8', '#F0FDF4', '#FDF2F8', '#ECFEFF'];
  ...
  const handleQuantityChange = (id: string, delta: number, currentQuantity: number, unit: string) => {
                    ...
                    {isEditing ? (
                      <div className="flex items-center gap-2">
                        <div className="flex items-center bg-white rounded-lg border border-gray-300 shadow-sm">
                          <button
                            onClick={() => handleQuantityChange(item.id, item.unit === 'g' ? -100 : -1, item.quantity, item.unit)}
        ...
        <div className="bg-white border-t border-gray-200 p-6 shrink-0 shadow-lg">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-right">
              ...
              <p className="text-sm font-semibold text-[#59A22F]">무료</p>
            </div>
          </div>
